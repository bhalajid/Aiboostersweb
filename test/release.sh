#!/usr/bin/env bash
# ---------------------------------------------------------------
# AIBoosters release gate. Nothing ships unless every stage passes.
#   ./release.sh <site-dir>
# ---------------------------------------------------------------
set -uo pipefail
DIR="${1:-..}"
...
step "1/5  JS syntax + tag balance"
node -e '
...
step "2/5  sphere hit-testing (real Three.js math)"
...
step "3/5  smoke test (jsdom, real page JS)"
...
step "4/5  accessibility + SEO audit"
python3 - "$DIR" <<'PY'
...
step "5/5  colour contrast (WCAG AA)"
python3 - "$DIR" <<'PY'
import re,sys,os
def lum(x):
